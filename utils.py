...
def download_model(save_dir = os.getcwd()+'/models/'):
    if not os.path.isfile(os.path.join(save_dir,'scaler.pkl')):
        ...
        url = 'https://github.com/vidhigupta9/aws/blob/main/models/scaler.pkl?raw=true'
    ...
    if not os.path.isfile(os.path.join(save_dir,'model_image.pt')):
        gdown.download(id='1-TI3lKVLkcl-5dF0-njvDxS_lyAQcZxp', output=os.path.join(save_dir,'model_image.pt'), quiet=False)
